...
Generator 函数有多种理解角度。**从语法上，Generator 函数是一个状态机**，封装了多个内部状态。
...
Generator 函数的调用方法与普通函数一样。不同的是，调用 Generator 函数后，该函数并不执行，返回的也不是函数运行结果，而是一个指向内部状态的指针对象，也就是上一章介绍的遍历器对象（Iterator Object）。
...
（5）如果Generator 函数已经运行完毕，`next`方法返回对象的`value`属性为`undefined`，`done`属性为`true`。以后再调用`next`方法，返回的都是这个值。
...
`forEach`等方法的参数也是一个普通函数，在里面使用`yield`表达式会报错。一种修改方法是改用`for`循环。
...
Generator 函数执行后，返回一个遍历器对象。该**遍历器对象本身也具有`Symbol.iterator`属性，执行后返回自身**。
...
这个功能有很重要的语法意义。Generator 函数从暂停状态到恢复运行，它的上下文状态（context）是不变的。通过`next`方法的参数，就可以在 Generator 函数运行的不同阶段，从外部向内部注入不同的值，从而调整函数行为。
...
需要注意，一旦`next`方法的返回对象的`done`属性为`true`，`for...of`循环就会中止，**且不包含该返回对象**。所以`return`语句返回的值，不包括在`for...of`循环之中。

**Generator 函数中最后一个`yield`语句返回对象的 value 为 false**。
...
原生的 JavaScript 对象没有遍历接口，无法使用`for...of`循环，通过 Generator 函数为它加上这个接口，就可以遍历任意对象（object）了。
